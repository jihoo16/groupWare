...
public interface ProjectFileRepository extends JpaRepository<ProjectFile, Long> {
     ...
     * 프로젝트별 파일 목록 조회
     */
    @Query("SELECT f FROM ProjectFile f WHERE f.projectIdx = :projectIdx AND (f.isDeleted IS NULL OR f.isDeleted = false) " +
     ...
     * 업로드 사용자별 파일 조회
     */
    @Query("SELECT f FROM ProjectFile f WHERE f.uploadUserIdx = :userIdx AND (f.isDeleted IS NULL OR f.isDeleted = false) " +
            "ORDER BY f.createdAt DESC")
