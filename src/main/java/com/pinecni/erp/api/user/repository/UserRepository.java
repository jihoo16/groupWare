...
public interface UserRepository extends JpaRepository<User, Long> {
     ...
     */
    @Query("SELECT u FROM User u WHERE u.deletedAt IS NULL AND u.isDev = false ORDER BY u.empId ASC")
    List<User> findAllActive();

    /**
     * 전체 사용자 조회 (삭제 포함, 개발자 계정 제외, 사번 오름차순)
     */
    @Query("SELECT u FROM User u WHERE u.isDev = false ORDER BY u.empId ASC")
    List<User> findAllNonDev();

    /**
     * 부서별 활성 사용자 조회 (개발자 계정 제외, 사번 오름차순)
     */
    @Query("SELECT u FROM User u WHERE u.empDept = :empDept AND u.deletedAt IS NULL AND u.isDev = false ORDER BY u.empId ASC")
    List<User> findActiveByEmpDept(String empDept);

    /**
     * 직급별 활성 사용자 조회 (개발자 계정 제외, 사번 오름차순)
     */
    @Query("SELECT u FROM User u WHERE u.empPosition = :empPosition AND u.deletedAt IS NULL AND u.isDev = false ORDER BY u.empId ASC")
    ...
    @Query("SELECT u FROM User u JOIN Code c ON c.groupCode = 'C02' AND c.code = u.empPosition " +
           "WHERE u.deletedAt IS NULL AND u.isDev = false AND c.sortOrder <= 3 ORDER BY c.sortOrder ASC, u.empId ASC")
    List<User> findActiveHighRankManagers();

    /**
     * 상태별 사용자 조회 (개발자 계정 제외, 사번 오름차순)
     */
    @Query("SELECT u FROM User u WHERE u.empStatus = :empStatus AND u.deletedAt IS NULL AND u.isDev = false ORDER BY u.empId ASC")
    List<User> findByEmpStatus(String empStatus);

    /**
     * 이름 검색 (개발자 계정 제외, 사번 오름차순)
     */
    @Query("SELECT u FROM User u WHERE u.empName LIKE %:name% AND u.deletedAt IS NULL AND u.isDev = false ORDER BY u.empId ASC")
    List<User> searchByName(String name);

    /**
     * 이름으로 활성 사용자 조회 (정확히 일치, 개발자 계정 제외)
     */
    @Query("SELECT u FROM User u WHERE u.empName = :empName AND u.deletedAt IS NULL AND u.isDev = false")
     ...
     */
    @Query("SELECT u FROM User u WHERE u.isTeamLeader = :isTeamLeader AND u.deletedAt IS NULL AND u.isDev = false ORDER BY u.empId ASC")
    List<User> findActiveByIsTeamLeader(Boolean isTeamLeader);

    /**
     * 상위보고자별 활성 사용자 조회 (개발자 계정 제외)
     */
    @Query("SELECT u FROM User u WHERE u.managerIdx = :managerIdx AND u.deletedAt IS NULL AND u.isDev = false ORDER BY u.empId ASC")
    ...
    @Query("SELECT COUNT(u) FROM User u JOIN Code c ON c.groupCode = 'C02' AND c.code = u.empPosition " +
           "WHERE u.managerIdx IS NULL AND u.deletedAt IS NULL AND u.isDev = false AND c.sortOrder > 1")
     ...
     */
    @Query("SELECT COUNT(u) FROM User u JOIN Code c ON c.groupCode = 'C02' AND c.code = u.empPosition " +
           "WHERE u.managerIdx IS NOT NULL AND u.deletedAt IS NULL AND u.isDev = false AND c.sortOrder > 1")
